...
Vue2.0教程：https://link.zhihu.com/?target=http%3A//cn.vuejs.org/


## 起步锁

扎实的 JavaScript / HTML / CSS 基本功。这是前置条件。
通读官方教程 (guide) 的基础篇。不要用任何构建工具，就只用最简单的script，把教程里的例子模仿一遍，理解用法。不推荐上来就直接用 vue-cli 构建项目，尤其是如果没有 Node\Webpack 基础。
...
参考 贡献指南 阅读 Vue 的源码，理解内部实现细节。（需要了解 Flow）
参与 Vue GitHub issue 的定位 -> 贡献 PR -> 加入核心团队 -> 升任 CTO ...